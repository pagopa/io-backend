  ...
  implements ISessionStorage
{
  ...
  private arrayStringReplyAsync(
    command: TE.TaskEither<Error, readonly (null | string)[]>,
  ...
  private loadSessionByToken(
    prefix: string,
    token: BPDToken | FIMSToken | MyPortalToken | WalletToken | ZendeskToken,
  ): Promise<Either<Error, User>> {
    return pipe(
      TE.tryCatch(
        () =>
  ...
  private mGet(keys: string[]): TaskEither<Error, (null | string)[]> {
    return pipe(
      keys,
      A.map((singleKey) =>
        TE.tryCatch(() => {
  ...
  private parseUserSessionList(
    userSessionTokensResult: readonly string[],
  ): SessionsList {
  ...
  public async clearExpiredSetValues(
    fiscalCode: string,
  ): Promise<readonly Either<Error, boolean>[]> {
  ...
  public async del(user: User): Promise<Either<Error, boolean>> {
    const tokens: readonly string[] = R.collect(
  ...
  public async delUserAllSessions(
    ...
    const delEverySession = (
      sessionTokens: readonly SessionToken[],
    ...
    return pipe(
      ...
      TE.map((keys) =>
        this.parseUserSessionList(
          keys.filter((key) => key !== null) as readonly string[],
        ),
      ),
    )();
  }
