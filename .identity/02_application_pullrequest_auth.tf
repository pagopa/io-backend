resource "azuread_directory_role_assignment" "pullrequest_directory_readers" {
  role_id             = azuread_directory_role.directory_readers.template_id
  principal_object_id = azuread_service_principal.pullrequest.object_id
}
...
resource "azurerm_role_assignment" "pullrequest_terraform_storage_account_app" {
  for_each             = { for s in local.subscriptions : s.name => s }
